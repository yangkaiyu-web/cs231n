...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        a = torch.arange(0, embed_dim)
        a[1::2] -= 1
        a = torch.exp(-a / embed_dim * torch.log(torch.tensor([10000])))
        pe[0] = a * (torch.arange(0, max_len).view(max_len,1))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        #  out:(N, H, S, T)
        #  mask:(T,S)
        out = torch.matmul(Q, K.permute(0, 1, 3, 2)) / self.scale
        if attn_mask is not None:
          out = torch.masked_fill(out, attn_mask == 0, -float('inf'))
        out = self.dropout(torch.softmax(out, dim=-1))

        output = torch.matmul(out, V).permute(0, 2, 1, 3).contiguous().view(N, S, D)
