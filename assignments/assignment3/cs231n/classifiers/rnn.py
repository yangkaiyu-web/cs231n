...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type=="rnn":
          dout2, dout1, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dout, cache3)
    ...
    def sample(self, features, max_length=30):
        ...
        x0 = self._start * np.ones(N, dtype=np.int8)
        h0, _ = affine_forward(features, W_proj, b_proj)
        for i in range(max_length):
          out1, _ = word_embedding_forward(x0, W_embed)
          h0, _ = rnn_step_forward(out1, h0, Wx, Wh, b)
          out4, _ = affine_forward(h0, W_vocab, b_vocab)
          x0 = np.argmax(out4, axis=1)
          captions[:, i] = x0
